...
class Matcher:
    ...
    def load(self, df):
        ...
        for relative_sno, df_per_sno in df_grouped:
            ...
            try:
                if __debug__:
                    _df_per_sno = copy.deepcopy(df_per_sno)
                    matcher.load(df_per_sno)
                    pd.testing.assert_frame_equal(_df_per_sno, df_per_sno)
                else:
                    matcher.load(df_per_sno)
            except Skip:
                continue
...
class MatcherPerSno:
    ...
    def __init__(self, matchers=(PhipsiMatcher, CovalentMatcher)):
