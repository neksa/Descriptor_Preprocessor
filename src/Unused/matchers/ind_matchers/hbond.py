...
class _HbMatcher:
    ...
    def load(self, hb_df):
        ...
        for component in self.components:
            ...
            if __debug__:
                _hb_df = copy.deepcopy(hb_df)
                weight = component.load(hb_df)
                assert len(hb_df) == len(_hb_df)
                for i in range(len(hb_df)):
                    pd.testing.assert_frame_equal(hb_df[i], _hb_df[i])
            else:
                weight = component.load(hb_df)
    ...
    def query(self, hb_datapoint):
        ...
        for component in self.components:
            if __debug__:
                _hb_datapoint = copy.deepcopy(hb_datapoint)
                raw_p = component.query(hb_datapoint)
                pd.testing.assert_frame_equal(hb_datapoint, _hb_datapoint)
            else:
                raw_p = component.query(hb_datapoint)
            name = component.__class__.__name__
