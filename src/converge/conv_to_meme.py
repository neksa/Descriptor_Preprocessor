...
def convert(input_conv_path, composition_path, output_path):
    ...
    num_matches, pssm, headers_pos = _parse_converge_output(input_conv_path)
    del headers_pos
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if not line.startswith(">"):
                ...
                continue
            header, motif_pos = line.rsplit(",", maxsplit=1)
            headers_pos[header] = int(motif_pos)
    pssm = np.array(pssm, dtype=int)
    return num_matches, pssm, headers_pos
