...
def read(handle):
    ...
    ...         print(instance.motif_name, instance.sequence_name,
    instance.strand, instance.pvalue)
    ...
    while True:
        motif_number, length, num_occurrences, evalue = __read_motif_statistics(
            line)
        name = __read_motif_name(handle)
        instances = __read_motif_sequences(handle, name, alphabet, length,
                                           revcomp)
        ...
        try:
            ...
            raise ValueError(
                'Unexpected end of stream: Expected to find new motif, '
                'or the summary of motifs')
...
class Instance(Seq.Seq):
    """A class describing the instances of a MEME motif, and the data
    thereof."""
...
def __read_version(record, handle):
    for line in handle:
        ...
        raise ValueError(
            "Improper input file. File should contain a line starting MEME "
            "version.")
    line = line.strip()
    ls = line.split()
    record.version = ls[2]
...
def __read_datafile(record, handle):
    for line in handle:
        ...
        raise ValueError(
            "Unexpected end of stream: 'TRAINING SET' not found. This can "
            "happen with " + "minimal MEME files (MEME databases) which are "
                             "not supported yet.")
    try:
        line = next(handle)
    except StopIteration:
        raise ValueError(
            "Unexpected end of stream: Expected to find line starting with "
            "'****'")
...
def __read_motif_statistics(line):
    ...
    #    MOTIF ATTATAAAAAAA MEME-1	width =  12  sites =   5  llr = 43
    #    E-value = 1.9e-003
    words = line.split()
...
def __skip_unused_lines(handle):
    ...
    try:
        ...
        raise ValueError(
            'Unexpected end of stream: Expected to find blank line')
    if line.strip():
