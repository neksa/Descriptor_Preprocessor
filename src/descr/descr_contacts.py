...
from .descr_config import HC_THRESHOLD, CC_THRESHOLD
...
def _get_HETATOM_contacts(a1_coords, a1_snos, a2_HETATM_coords, dsr_snos):
    ...
    contact_mask = distance < HC_THRESHOLD
    covalent_mask = distance < CC_THRESHOLD
...
def _get_descr_contacts_HETATOM_bool(a1_coords, a1_snos, a2_HETATM_coords,
                                    dsr_snos):
    """
    DEPRECATED.

    Replaced in use by _get_HETATOM_contacts, no longer maintained.
    ...
    # a2_HETATM_coords.shape = (5, 3)
    # value.shape = (258, 5, 3)
    value = np.array([a1 - a2_HETATM_coords for a1 in a1_coords])
    ...
    # If any below HC_THRESHOLD, relevant a1_sno has contact (with anything)
    # con_sno_mask.shape = (258)
    con_sno_mask = np.apply_along_axis(any, 1, contact_mask)
    cov_sno_mask = np.apply_along_axis(any, 1, covalent_mask)

    # masked_array mask if True, hence need to reverse, to mask the False ones
    # matched_con_snos.shape = (258)
    matched_con_snos = np.ma.masked_array(a1_snos, mask=~con_sno_mask)
    matched_cov_snos = np.ma.masked_array(a1_snos, mask=~cov_sno_mask)
    _hetatom_contacts = set(np.ma.compressed(matched_con_snos))
    _hetatom_covalent = set(np.ma.compressed(matched_cov_snos))

    hetatom_contacts = dict()
    hetatom_contacts['sno'] = dsr_snos
    hetatom_contacts['contact'] = [True if sno in _hetatom_contacts
                                   else False for sno in dsr_snos]

    hetatom_covalent = dict()
    hetatom_covalent['sno'] = dsr_snos
    hetatom_covalent['covalent'] = [True if sno in _hetatom_covalent
                                    else False for sno in dsr_snos]
